...
export default function Loan({
  ...
  const chartData = {
    labels: schedule.map(entry => `${t.month} ${entry.month}`),
    datasets: [
      {
        label: t.beginningBalance,
        data: schedule.map(entry => entry.beginningBalance),
        borderColor: 'rgba(75, 192, 192, 1)',
        backgroundColor: 'rgba(75, 192, 192, 0.2)',
      },
      {
        label: t.endingBalance,
        data: schedule.map(entry => entry.endingBalance),
